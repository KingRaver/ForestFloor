...
constexpr const char* kPluginId = "ff.test.missing-symbols";
constexpr const char* kPluginName = "Test Missing Symbol Plugin";
...
FF_TEST_PLUGIN_EXPORT bool ff_plugin_get_binary_info_v1(
  ...
  out_binary_info->plugin_class =
      static_cast<std::uint32_t>(ff::plugin_host::PluginClass::kInstrument);
  out_binary_info->entrypoints = ff::plugin_host::PluginEntrypointsC{
      .has_create = 1,
      .has_prepare = 1,
      .has_process = 1,  // claims process exists, but symbol is intentionally missing
      .has_reset = 1,
      .has_destroy = 1,
  };
  out_binary_info->runtime = ff::plugin_host::PluginRuntimeInfoC{
      ...
      .requests_process_isolation = 0,
      .has_unbounded_cpu_cost = 0,
  };
  return true;
}
